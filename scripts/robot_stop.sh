#!/bin/bash
# robot_stop.sh — stop exploration early but keep SLAM running, so the map can still be saved.
...
timeout 10 ros2 service call /control_exploration frontier_exploration_ros2/srv/ControlExploration \
  ...
  || echo "robot_stop: explorer control service did not answer" >&2
...
timeout 10 ros2 service call /navigate_to_pose/_action/cancel_goal action_msgs/srv/CancelGoal "{}" \
  || echo "robot_stop: Nav2 cancel did not answer" >&2
...
timeout 20 ros2 topic pub -w 1 -r 10 -t 10 /cmd_vel geometry_msgs/msg/TwistStamped \
  ...
  || echo "robot_stop: zero velocity not sent (no /cmd_vel subscriber found)" >&2

echo "robot_stop: done. Save the map with scripts/robot_save_map.sh before closing the launch."
